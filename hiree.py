...
def get_all_resumes_links(query, area, number_of_links):
    ...
    for i in range(number_of_links//20):
...
def apply_tags(progress: int):
    ...
    with open("good_resumes.txt", 'r', encoding='utf-8') as f:
        for link in tqdm(f, total=progress):
            total = 0
            try:
                html = get_html(link, False)
                soup = BeautifulSoup(html, 'lxml')
...
if __name__ == '__main__':
    ...
    links = get_all_resumes_links(query, area, 100)
